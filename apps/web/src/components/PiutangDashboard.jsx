...
const PiutangDashboard = () => {
  ...
  const handlePaySupplier = async () => {
    ...
    try {
      await axios.put(`${baseURL}/api/purchases/${payModal.id}/payment`, {
        totalBayar: safeNum(payAmount), buktiBayar: buktiTf, tanggal: payDate
      });
  ...
  const handlePayCustomer = async () => {
    ...
    setIsLoading(true);
    try {
      const newDp = safeNum(payAmount); 
      const sisa = payModal.grandTotal - newDp;
      await axios.put(`${baseURL}/api/orders/${payModal.id}/payment`, {
        status: sisa <= 0 ? 'SELESAI' : 'TERKIRIM', dp: newDp, buktiLunas: buktiTf, tanggal: payDate 
      });
  ...
  const openAddPurchase = () => {
    setPurchaseForm({
      id: null, supplier: null, tanggal: new Date().toISOString().split('T')[0], tglJatuhTempo: '', items: [], bayar: 0, keterangan: '', buktiNota: '', buktiTf: '', metodeBayar: 'TF'
    });
  ...
  const openEditPurchase = (p) => {
    ...
    setPurchaseForm({
      ...
      items: (p.items || []).map(i => ({
        productId: i.productId, nama: i.product?.nama || 'Unknown',
        satuanBeli: i.product?.satuanBeli || '-', satuanJual: i.product?.satuanJual || '-',
  ...
  const handleSimpanBarangMasuk = async () => {
    ...
    try {
      const payload = {
        ...
        totalBayar: safeNum(purchaseForm.bayar), metodeBayar: purchaseForm.metodeBayar,
  ...
  const renderLinks = (textString, isLunas) => {
    ...
    const links = textString.split(/\s+/).filter(l => l.trim() !== '');
    return links.map((link, idx) => ( 
      <a key={idx} href={link.startsWith('http') ? link : `https://${link}`} target="_blank" rel="noreferrer" className={`${isLunas ? 'text-gray-500 bg-white' : 'text-blue-600 bg-white'} p-2 rounded-lg inline-flex shadow-sm hover:opacity-80 transition-opacity border border-gray-200`}>
        <LinkIcon size={14}/>
      </a>
    ));
  };
  ...
  return (
    <div className="space-y-3 md:space-y-5 h-full flex flex-col">
      ...
      <div className="flex flex-col md:flex-row gap-3 shrink-0">
        ...
        <button onClick={handleExportPiutang} className="bg-green-600 hover:bg-green-700 text-white py-3 px-5 rounded-xl font-bold flex justify-center items-center gap-2 shadow-sm transition-transform active:scale-95 text-xs md:text-sm whitespace-nowrap"><Download size={18}/> Export Piutang</button>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm flex-1 flex flex-col">
        <div className="p-4 border-b flex flex-col lg:flex-row gap-3 items-center justify-between bg-gray-50/50">
          <div className="flex flex-wrap lg:flex-nowrap items-center gap-2 w-full lg:w-auto">
            {activeTab === 'supplier' && (
              <button onClick={openAddPurchase} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-sm transition-transform active:scale-95 w-full lg:w-auto whitespace-nowrap"><PackagePlus size={16}/> Input Barang Masuk</button>
            )}
            ...
            {activeTab === 'supplier' && (
              <select className="border bg-white p-2.5 rounded-xl text-xs font-bold text-gray-600 outline-none flex-1 lg:flex-none shadow-sm cursor-pointer" value={filterSupplier} onChange={(e) => setFilterSupplier(e.target.value)}>
                <option value="">Semua Supplier</option>
                {suppliers.map(s => <option key={s.id} value={s.id}>{s.nama} (#{s.id})</option>)}
              </select>
            )}
          ...
          <div className="relative w-full lg:w-64 mt-2 lg:mt-0">
            ...
            <input className="pl-9 pr-4 py-2.5 border rounded-xl w-full text-xs outline-none focus:border-blue-500 shadow-sm bg-white" placeholder="Cari ID / Nama..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        </div>
              ...
              {activeTab === 'customer' && filteredOrders.map(o => {
                ...
                return (
                  <tr key={o.id} className={`transition-colors ${isLunas ? 'bg-green-50/50 text-gray-500' : 'hover:bg-blue-50/30'}`}>
                    ...
                    <td className="p-4 text-center text-[12px] font-bold text-green-700 bg-green-50/20">{o.lastPaymentDate ? new Date(o.lastPaymentDate).toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '-'}</td>
                ...
                return (
                  <tr key={p.id} className={`transition-colors ${isLunas ? 'bg-green-50/50 text-gray-500' : 'hover:bg-red-50/30'}`}>
                    ...
                    <td className="p-4 text-center space-y-2">
                      {isLunas ? (
                        <button onClick={() => { setPayModal(p); setPayAmount(p.totalBayar); setBuktiTf(p.buktiBayar || ''); setPayDate(p.lastPaymentDate ? p.lastPaymentDate.split('T')[0] : new Date().toISOString().split('T')[0]); }} className="text-green-700 font-bold text-[11px] bg-white border border-green-200 px-3 py-2 rounded-xl hover:bg-green-50 inline-flex items-center gap-1.5 w-full justify-center"><CheckCircle size={14}/> LUNAS</button>
                      ) : (
            ...
            <div className="p-4 md:p-6 space-y-5 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 border-b pb-5">
                ...
                <div className="col-span-5 grid grid-cols-2 gap-2 bg-red-50 border border-red-100 p-2 rounded-lg">
                  ...
                  <div>
                    <label className="block text-[9px] font-bold text-red-600 mb-1 uppercase">Jatuh Tempo</label>
                    <input type="date" className="w-full border border-red-200 bg-white p-1 rounded text-[11px] font-bold outline-none focus:border-red-500" value={purchaseForm.tglJatuhTempo} onChange={e => setPurchaseForm({ ...purchaseForm, tglJatuhTempo: e.target.value })} />
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-3 items-end mb-4">
                  ...
                  <div className="w-24">
                    ...
                    <div className="flex bg-white border-2 rounded-lg focus-within:border-red-500 overflow-hidden">
                      <input type="text" className="w-full p-2 outline-none text-sm font-bold text-red-700 text-center" value={qtyBeli} onChange={e => setQtyBeli(formatQtyInput(e.target.value))} placeholder="0" />
                  ...
                  <div className="w-32">
                    ...
                    <input type="text" className="w-full border-2 p-2 rounded-lg outline-none focus:border-red-500 text-sm font-bold text-red-700" value={formatRupiahInput(hargaBeli)} onChange={e => setHargaBeli(parseRupiahInput(e.target.value))} placeholder="Rp" />
                  </div>
                  <div className="w-28">
                    ...
                    <div className="flex bg-white border-2 rounded-lg focus-within:border-green-500 overflow-hidden">
                      <input type="text" className="w-full p-2 outline-none text-sm font-bold text-green-700 text-center" value={qtyJual} onChange={e => setQtyJual(formatQtyInput(e.target.value))} placeholder="0" />
              ...
              <div className="flex flex-col sm:flex-row gap-5">
                <div className="flex-1 space-y-4">
                  <div>
                    ...
                    <textarea className="w-full border-2 p-3 rounded-xl h-[80px] outline-none text-xs focus:border-blue-500 resize-none shadow-sm" value={purchaseForm.keterangan} onChange={e => setPurchaseForm({ ...purchaseForm, keterangan: e.target.value })} placeholder="Ketik catatan tambahan..."></textarea>
                  </div>
                  <div className="bg-gray-50 p-3.5 rounded-xl border border-gray-200 shadow-sm">
                    <label className="text-[10px] font-bold text-gray-700 mb-1.5 flex items-center gap-1.5 uppercase"><LinkIcon size={14} className="text-blue-500"/> Link Foto Nota / Surat Jalan</label>
                    <textarea placeholder="Paste link Drive / Link Foto di sini...&#10;(Pisahkan dgn SPASI)" value={purchaseForm.buktiNota || ''} onChange={(e) => setPurchaseForm(prev => ({ ...prev, buktiNota: e.target.value }))} className="text-xs w-full bg-white p-2.5 rounded-lg border-2 outline-none focus:border-blue-500 h-[65px] resize-none"></textarea>
                  </div>
                </div>
                <div className="flex-1 bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-center space-y-3">
                  ...
                  <div className="flex justify-between font-bold text-green-700 items-center gap-2">
                    ...
                    <input type="text" className="w-1/2 border-2 p-2.5 rounded-lg text-right font-black bg-green-50 outline-none focus:border-green-500 text-sm" value={formatRupiahInput(purchaseForm.bayar)} onChange={e => setPurchaseForm({ ...purchaseForm, bayar: parseRupiahInput(e.target.value) })} placeholder="0" />
                  </div>
        ...
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[10000] p-4 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl shadow-2xl w-[400px] border border-gray-100">
            ...
            <div className="mb-5 bg-gray-50 p-4 rounded-xl border border-gray-200">
              <p className="text-sm font-bold text-gray-800 uppercase">{activeTab === 'customer' ? `${payModal.customer?.nama} (#${payModal.customerId})` : `${payModal.supplier?.nama} (#${payModal.supplierId})`}</p>
            ...
            <div className="mb-4">
              ...
              <input type="text" className="w-full border-2 p-3 rounded-xl font-black text-lg outline-none focus:border-green-500 text-green-700 bg-green-50/50 shadow-sm" value={formatRupiahInput(payAmount)} onChange={e => setPayAmount(parseRupiahInput(e.target.value))} />
        ...
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[10000] p-4 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-[400px]">
            <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-3">
              <div>
                <h3 className="font-bold text-base text-gray-900 flex items-center gap-2">Catatan Transaksi <FileText size={18} className="text-yellow-500"/></h3>
                <p className="text-[10px] text-gray-500 font-semibold uppercase mt-1">Oleh: <span className="text-blue-600">{noteModal?.nama}</span></p>
              </div>
