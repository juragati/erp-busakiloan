...
const OrderList = () => {
  ...
  const [form, setForm] = useState({
    ...
    items: [], dp: '', ongkir: '', ongkirModal: '', metodeBayar: 'TF',
    buktiLunas: '', catatan: '', tanggalJatuhTempo: ''
  });
  ...
  const fetchCustomers = () => axios.get(`${baseURL}/api/customers`).then(res => {
    if (Array.isArray(res.data)) setCustomers(res.data.map(c => ({ value: c.id, label: `${c.nama} (#${c.id}) ${c.alamat ? `(${c.alamat})` : ''}`, dataAsli: c })));
  }).catch(console.error);
  ...
  const openAddModal = () => {
    ...
    setSelectedProduct(null); setHargaSatuan(''); setHppSatuan('');
    setQty(1); setIsNewProduct(false); setIsEditing(false);
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (o) => {
    ...
    let safeTanggal = '';
    try { safeTanggal = o.tanggal ? new Date(o.tanggal).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]; }
    catch (e) { safeTanggal = new Date().toISOString().split('T')[0]; }

    let safeJatuhTempo = '';
    try { if (o.tanggalJatuhTempo) safeJatuhTempo = new Date(o.tanggalJatuhTempo).toISOString().split('T')[0]; }
    catch (e) {}

    setForm({
      id: o.id, customerId: fullCustomerData, sopirId: fullSopirData,
      namaPelanggan: o.customer?.nama, tanggal: safeTanggal,
      tanggalJatuhTempo: safeJatuhTempo, status: o.status || 'MENUNGGU',
      items: o.items.map(i => ({
      ...
      dp: o.dp || '', ongkir: o.ongkosKirim || '', ongkirModal: o.ongkosKirimModal || '', metodeBayar: o.metodeBayar || 'TF',
      buktiLunas: o.buktiLunas || o.buktiDp || '', catatan: o.keterangan || ''
    });
    
    setSelectedProduct(null); setHargaSatuan(''); setHppSatuan('');
    setQty(1); setIsNewProduct(false); setIsEditing(true);
    setIsModalOpen(true);
  };

  const handleCreateCustomer = async (inputValue) => {
    try {
      const res = await axios.post(`${baseURL}/api/customers/upsert`, {
        nama: inputValue, ongkirDefault: 0, ongkirPerusahaanDefault: 0
      });
  ...
  const handleCreateProduct = async (inputValue) => {
    try {
      const res = await axios.post(`${baseURL}/api/products/upsert`, {
        nama: inputValue, hargaJual: 0, hpp: 0, stok: 0, isHppManual: false
      });
      const newProd = { ...res.data, value: res.data.id, label: `${res.data.nama} (#${res.data.id})`, dataAsli: res.data };
      setProducts(prev => [...prev, newProd]);
      setSelectedProduct(newProd); setHargaSatuan(''); setHppSatuan(''); setIsNewProduct(true);
  ...
  const handleProductChange = (selectedProd) => {
    ...
    if (selectedProd) {
      ...
      setHargaSatuan(''); setHppSatuan('');
    }
  };

  const handleAddItem = async (e) => {
    e.preventDefault();
    const qtyNum = parseQtyInput(qty); 
    const hargaNum = parseFloat(hargaSatuan) || 0; 
    const hppNum = parseFloat(hppSatuan) || 0;

    if (!selectedProduct) return alert("Silakan pilih produk terlebih dahulu!");
    if (qtyNum <= 0) return alert("Jumlah (Qty) harus lebih dari 0!");
    ...
    if (isNewProduct) {
      try {
        await axios.post(`${baseURL}/api/products/upsert`, {
          id: selectedProduct.value, nama: selectedProduct.dataAsli?.nama || selectedProduct.label,
          hargaJual: hargaNum, hpp: hppNum
        });
        fetchProducts();
    ...
    const newItem = {
      ...
      qty: qtyNum, hargaJual: hargaNum, hppSatuan: hppNum, subtotal: hargaNum * qtyNum
    };
  ...
  // Real-time Profit Estimasi logic (sama dengan Profit Dashboard)
  const calculateEstimasiProfit = () => {
    ...
    form.items.forEach(item => {
      const hpp = parseFloat(item.hppSatuan || 0);
      const jual = parseFloat(item.hargaJual || 0);
      const q = parseFloat(item.qty || 0);
      
      // Barang dengan HPP 0 dilewati untuk estimasi profit
      if (hpp > 0) {
        orderOmsetBarang += (jual * q);
        orderModalBarang += (hpp * q);
      }
    });

    return (orderOmsetBarang - orderModalBarang) + (ongkirNum - ongkirModalNum);
  };
  ...
  const handleQuickStatusChange = async (order, newStatus) => {
    ...
    try {
      const payload = {
        status: newStatus, dp: order.dp, tanggal: order.tanggal,
        tanggalJatuhTempo: order.tanggalJatuhTempo, keterangan: order.keterangan,
        items: order.items.map(i => ({ productId: i.productId, qty: i.qty, hargaJual: i.hargaSatuan, hppSatuan: i.hppSatuan })),
        ongkosKirim: order.ongkosKirim, ongkosKirimModal: order.ongkosKirimModal,
        metodeBayar: order.metodeBayar, sopirId: order.sopirId, buktiLunas: order.buktiLunas
      };
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      const payload = {
        status: form.status, dp: dpNum, tanggal: form.tanggal,
        tanggalJatuhTempo: form.tanggalJatuhTempo, keterangan: form.catatan,
        items: form.items, ongkosKirim: ongkirNum, ongkosKirimModal: ongkirModalNum,
        metodeBayar: form.metodeBayar, sopirId: form.sopirId?.value || null,
        buktiLunas: form.buktiLunas, customerId: form.customerId.value,
      ...
      alert("Gagal menyimpan order: " + (e.response?.data?.error || e.message));
            ...
            {processedOrders.map(o => (
              <tr key={o.id} className="hover:bg-blue-50/30 transition-colors">
                ...
                <td className="p-4 text-center">
                  <select value={o.status} onChange={(e) => handleQuickStatusChange(o, e.target.value)} className={`px-2 py-1.5 pr-6 rounded-lg text-xs font-bold uppercase tracking-wider outline-none cursor-pointer border shadow-sm transition-colors hover:brightness-95 ${getStatusBadge(o.status)}`}>
              ...
              <div className="bg-white p-4 rounded-2xl border shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="lg:col-span-2">
                    ...
                    <CreatableSelect
                      options={customers} value={form.customerId} onChange={handleCustomerChange} onCreateOption={handleCreateCustomer}
                  ...
                  <div className="lg:col-span-2">
                    ...
                    <CreatableSelect
                      isClearable options={sopirs} value={form.sopirId} onChange={handleSopirChange} onCreateOption={handleCreateSopir}
                  ...
                  <div className="lg:col-span-2">
                    <label className="text-xs font-semibold block mb-1">Tanggal Transaksi</label>
                    <input type="date" className="w-full border-2 p-2.5 rounded-xl text-sm" value={form.tanggal} onChange={e => setForm({ ...form, tanggal: e.target.value })} />
                  </div>
                  <div className="lg:col-span-2">
                    <label className="text-xs font-semibold text-red-600 block mb-1">Jatuh Tempo</label>
                    <input type="date" className="w-full border-2 p-2.5 rounded-xl text-sm border-red-200 text-red-700 bg-red-50 font-medium" value={form.tanggalJatuhTempo} onChange={e => setForm({ ...form, tanggalJatuhTempo: e.target.value })} />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-4 rounded-2xl border shadow-sm">
                ...
                <div className="flex flex-col md:flex-row gap-3 items-end mb-4">
                  <div className="w-full md:flex-1">
                    ...
                    <CreatableSelect
                      options={products} value={selectedProduct} onChange={handleProductChange} onCreateOption={handleCreateProduct}
                  ...
                  <div className="flex gap-2 w-full md:w-auto">
                    <div className="flex-1 md:w-32">
                      ...
                      <input type="text" className="w-full border-2 p-2.5 rounded-xl text-sm font-semibold outline-none focus:border-red-500 text-red-700" 
                             value={formatRupiahInput(hppSatuan)} onChange={e => setHppSatuan(parseRupiahInput(e.target.value))} placeholder="0" />
                    </div>
                    <div className="flex-1 md:w-32">
                      <label className="text-[10px] font-semibold text-gray-600 mb-1 block uppercase">Harga Jual</label>
                      <input type="text" className="w-full border-2 p-2.5 rounded-xl text-sm font-semibold outline-none focus:border-blue-500 text-blue-700" 
                             value={formatRupiahInput(hargaSatuan)} onChange={e => setHargaSatuan(parseRupiahInput(e.target.value))} placeholder="0" />
                    </div>
                    <div className="w-24">
                      ...
                      <div className="flex bg-white border-2 rounded-xl focus-within:border-blue-500 overflow-hidden h-[42px]">
                        <input type="text" className="w-full p-2 text-sm font-bold text-center outline-none" value={qty} onChange={e => setQty(formatQtyInput(e.target.value))} placeholder="1" />
                        <span className="bg-gray-100 text-[10px] font-bold text-gray-500 flex items-center px-1.5 border-l uppercase truncate max-w-[40px]">
                          {selectedProduct ? (selectedProduct.dataAsli?.satuanJual || '-') : '-'}
                        </span>
                      </div>
                    </div>
                    <button type="button" onClick={handleAddItem} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm flex items-center justify-center gap-2 flex-1 sm:flex-none">
                      ...
                      {form.items.map((i, idx) => (
                        <tr key={idx} className="hover:bg-gray-50">
                          ...
                          <td className="p-3 text-center">
                            <button onClick={() => handleRemoveItem(idx)} className="text-red-500 hover:bg-red-50 p-1.5 rounded-lg"><Trash2 size={16} /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                <div className="lg:col-span-3 bg-white p-4 rounded-2xl border space-y-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-orange-50 border border-orange-200 p-3 rounded-xl">
                      ...
                      <input type="text" className="w-full border-2 p-2 rounded-lg text-sm bg-white font-bold text-orange-700 outline-none" 
                             value={formatRupiahInput(form.ongkir)} onChange={e => setForm({ ...form, ongkir: parseRupiahInput(e.target.value) })} placeholder="0" title="Dibayar oleh pelanggan ke kita" />
                    </div>
                    <div className="bg-red-50 border border-red-200 p-3 rounded-xl">
                      <label className="text-[10px] font-bold text-red-800 uppercase block mb-1">Ongkir ke Sopir</label>
                      <input type="text" className="w-full border-2 p-2 rounded-lg text-sm bg-white font-bold text-red-700 outline-none" 
                             value={formatRupiahInput(form.ongkirModal)} onChange={e => setForm({ ...form, ongkirModal: parseRupiahInput(e.target.value) })} placeholder="0" title="Uang jalan yang dibayar ke sopir" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 pt-2">
                    <div>
                      <label className="text-xs font-bold text-gray-600 block mb-1">Sudah Bayar (DP)</label>
                      <input type="text" className="w-full border-2 p-2.5 rounded-xl border-green-200 text-green-700 bg-green-50 font-bold outline-none" 
                             value={formatRupiahInput(form.dp)} onChange={e => setForm({ ...form, dp: parseRupiahInput(e.target.value) })} />
                    </div>
                    <div>
                ...
                <div className="lg:col-span-2 bg-blue-50/50 p-5 rounded-2xl border-2 border-blue-100 flex flex-col justify-between">
                  <div>
                    ...
                    {/* Estimasi Profit UI Real-time */}
                    <div className={`mt-4 p-4 rounded-xl flex justify-between items-center border ${calculateEstimasiProfit() > 0 ? 'bg-blue-100 border-blue-200 text-blue-800' : 'bg-gray-100 border-gray-200 text-gray-800'}`}>
                      <div>
                         <span className="text-sm font-bold flex items-center gap-1">📊 Estimasi Profit:</span>
                      </div>
                      <span className="text-xl font-black">{formatRp(calculateEstimasiProfit())}</span>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[10000] p-4 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-[400px]">
            <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-3">
              <div>
                <h3 className="font-bold text-base text-gray-900 flex items-center gap-2">Catatan Order <FileText size={18} className="text-yellow-500" /></h3>
