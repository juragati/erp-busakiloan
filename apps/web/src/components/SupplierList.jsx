...
import { formatRupiahInput, parseRupiahInput, formatQtyInput, parseQtyInput, formatRp } from '../utils/currencyFormatter';
...
const SupplierList = () => {
  ...
  useEffect(() => { fetchSuppliers(); fetchCategories(); fetchAllProducts(); }, []);

  useEffect(() => {
    setFilteredSuppliers(suppliers.filter(s => {
      const st = searchTerm.toLowerCase();
      return (s.id?.toString().includes(st)) || (s.nama && s.nama.toLowerCase().includes(st)) || (s.alamat && s.alamat.toLowerCase().includes(st)) || (s.kontak && s.kontak.toLowerCase().includes(st));
    }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsModalOpen(false); fetchSuppliers(); alert("Supplier berhasil disimpan!");
    } catch(e) { alert("Gagal menyimpan supplier: \n" + (e.response?.data?.error || e.message)); }
  ...
  const openEditProduct = (p) => {
    setProductForm({
      ...
      hargaJual: p.hargaJual, hpp: p.hpp || 0, 
      stok: p.stok ? p.stok.toString().replace('.', ',') : '', // Format ke koma agar bisa di-parse
  ...
  const handleSaveProduct = async () => {
    ...
    setIsLoading(true);
    try {
      const payload = { ...productForm, stok: parseQtyInput(productForm.stok) };
      await axios.post(`${baseURL}/api/products/upsert`, payload);
  ...
  const handleCreateSupplier = async (val) => { 
    ...
    setSuppliers(p => [...p, { value: res.data.id, label: `${res.data.nama} (#${res.data.id})` }]); 
    setPurchaseForm(p => ({ ...p, supplier: { value: res.data.id, label: `${res.data.nama} (#${res.data.id})` } })); 
  };
  ...
  const handleAddItemToPurchase = () => {
    const qBeli = parseQtyInput(qtyBeli);
    const qJual = parseQtyInput(qtyJual);
    const hBeli = parseRupiahInput(hargaBeli);
    ...
    const newItem = {
      productId: selectedProduct.value, nama: selectedProduct.dataAsli?.nama || selectedProduct.label,
      satuanBeli: selectedProduct.dataAsli?.satuanBeli || '-', satuanJual: selectedProduct.dataAsli?.satuanJual || '-',
      qtyBeli: qBeli, qty: qJual, hargaBeli: hBeli, subtotal: qBeli * hBeli
    };
  ...
  const handleSetTempo = (days) => { 
    if (!days) return setPurchaseForm(p => ({ ...p, tglJatuhTempo: '' })); 
    const d = purchaseForm.tanggal ? new Date(purchaseForm.tanggal) : new Date();
    d.setDate(d.getDate() + parseInt(days)); 
    setPurchaseForm(p => ({ ...p, tglJatuhTempo: d.toISOString().split('T')[0] })); 
  };

  const handleSetFixedDate = (dayStr) => { 
    if (!dayStr) return; 
    const t = parseInt(dayStr); 
    const d = purchaseForm.tanggal ? new Date(purchaseForm.tanggal) : new Date(); 
    let y = d.getFullYear(); let m = d.getMonth(); 
    if (t <= d.getDate()) { m++; if (m > 11) { m = 0; y++; } }
    let nd = new Date(y, m, t); if (nd.getMonth() !== m) nd = new Date(y, m + 1, 0);
    const pad = (n) => n.toString().padStart(2, '0');
    setPurchaseForm(p => ({ ...p, tglJatuhTempo: `${nd.getFullYear()}-${pad(nd.getMonth() + 1)}-${pad(nd.getDate())}` })); 
  };
  ...
  const handleSimpanBarangMasuk = async () => {
    ...
    try {
      await axios.post(`${baseURL}/api/purchases`, {
        ...
        tanggal: purchaseForm.tanggal || new Date().toISOString(), tanggalJatuhTempo: purchaseForm.tglJatuhTempo || null,
        totalBayar: parseRupiahInput(purchaseForm.bayar), keterangan: purchaseForm.keterangan, buktiNota: purchaseForm.buktiNota
      });
    ...
    <div className="flex flex-col h-full space-y-4">
      ...
      <div className="bg-white p-4 border flex flex-col sm:flex-row justify-between items-center gap-4 rounded-xl shadow-sm shrink-0">
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <button onClick={openAddModal} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-bold flex justify-center items-center gap-2 shadow-sm transition-transform active:scale-95"><UserPlus size={18}/> Tambah Supplier</button>
          <button onClick={handleExportExcel} className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg font-bold flex justify-center items-center gap-2 shadow-sm transition-transform active:scale-95"><Download size={18}/> Export Excel</button>
        </div>
        ...
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[9999] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[500px] flex flex-col overflow-hidden">
            ...
            <div className="p-4 border-t bg-gray-50 flex gap-3 shrink-0">
              <button onClick={() => setIsModalOpen(false)} className="flex-1 py-2.5 bg-white border border-gray-300 rounded-lg text-sm font-bold text-gray-600 hover:bg-gray-100 transition-colors">Batal</button>
              <button onClick={handleSave} disabled={isLoading} className={`flex-1 py-2.5 text-white rounded-lg text-sm font-bold flex justify-center items-center gap-2 transition-transform active:scale-95 disabled:cursor-not-allowed ${isLoading ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'}`}><Save size={18}/> {isLoading ? 'Menyimpan...' : 'Simpan'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[9990] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[850px] flex flex-col max-h-[90vh] overflow-hidden">
            <div className="p-4 border-b flex justify-between items-center bg-gray-50 shrink-0">
              <div><h3 className="font-bold text-lg text-gray-800 flex items-center gap-2"><Package size={20} className="text-blue-600"/> Produk dari {detailSupplier.nama}</h3></div>
                ...
                <tbody className="divide-y">
                  {suppliedProducts.map(p => (
                    <tr key={p.id} className="hover:bg-gray-50">
                      <td className="p-4 font-bold text-gray-900 flex flex-col gap-0.5">{p.nama} <span className="text-[10px] text-gray-400">(#{p.id})</span><span className="text-[9px] text-gray-500 font-medium bg-gray-100 px-2 py-0.5 rounded w-max uppercase">Pabrik: {p.satuanBeli || '-'} | Customer: {p.satuanJual || '-'}</span></td>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-[500px] flex flex-col overflow-hidden">
            ...
            <div className="p-3 md:p-4 space-y-3 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3 bg-blue-50 p-3 rounded-lg border border-blue-100">
                <div><label className="text-[10px] font-bold text-blue-800 mb-1 block uppercase">Satuan Beli Pabrik</label><input list="satuanList" className="border-2 border-blue-200 p-2 rounded-lg w-full text-xs font-bold text-gray-800 outline-none focus:border-blue-500" value={productForm.satuanBeli} onChange={e => setProductForm({ ...productForm, satuanBeli: e.target.value })} placeholder="Ketik: kg, dll" /></div>
                <div><label className="text-[10px] font-bold text-green-800 mb-1 block uppercase">Satuan Jual Customer</label><input list="satuanList" className="border-2 border-green-200 p-2 rounded-lg w-full text-xs font-bold text-gray-800 outline-none focus:border-green-500" value={productForm.satuanJual} onChange={e => setProductForm({ ...productForm, satuanJual: e.target.value })} placeholder="Ketik: meter, dll" /></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 bg-gray-50 p-3 rounded-lg border">
                <div><label className="text-[9px] font-bold text-gray-500 mb-1 block uppercase">Harga Jual (per {productForm.satuanJual || '-'})</label><input className="border-2 p-2 rounded-lg w-full text-xs font-bold text-blue-600 outline-none focus:border-blue-500" type="text" value={formatRupiahInput(productForm.hargaJual)} onChange={e => setProductForm({ ...productForm, hargaJual: parseRupiahInput(e.target.value) })} /></div>
                <div><label className="text-[9px] font-bold text-red-500 mb-1 block uppercase">HPP Modal (per {productForm.satuanJual || '-'})</label><input className="border-2 p-2 rounded-lg w-full text-xs font-bold text-red-600 bg-white outline-none focus:border-red-500" type="text" value={formatRupiahInput(productForm.hpp)} onChange={e => setProductForm({ ...productForm, hpp: parseRupiahInput(e.target.value) })} /></div>
              </div>
              <div><label className="text-[10px] font-bold text-gray-500 mb-1 block">Stok Awal di Gudang ({productForm.satuanJual || '-'})</label><input className="border-2 p-2 rounded-lg w-1/2 text-xs outline-none focus:border-green-500" type="text" value={formatQtyInput(productForm.stok)} onChange={e => setProductForm({ ...productForm, stok: parseQtyInput(e.target.value) })} /></div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-[850px] max-h-[95vh] overflow-hidden flex flex-col">
            ...
            <div className="p-5 space-y-5 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 border-b pb-5">
                <div className="col-span-4"><label className="block text-[10px] font-bold text-gray-600 mb-1 uppercase">Supplier Pabrik</label><CreatableSelect options={suppliers.map(s => ({ value: s.id, label: `${s.nama} (#${s.id})` }))} value={purchaseForm.supplier} onChange={s => setPurchaseForm({ ...purchaseForm, supplier: s })} onCreateOption={handleCreateSupplier} placeholder="Ketik nama supplier..." /></div>
                <div className="col-span-3"><label className="block text-[10px] font-bold text-gray-600 mb-1 uppercase">Tgl Beli</label><input type="date" className="w-full border-2 p-[9px] rounded-md outline-none text-sm focus:border-green-500" value={purchaseForm.tanggal} onChange={e => setPurchaseForm({ ...purchaseForm, tanggal: e.target.value })}/></div>
              ...
              <div className="bg-orange-50 p-4 rounded-xl border border-orange-200">
                ...
                <div className="flex flex-wrap gap-3 items-end mb-4">
                  <div className="flex-1 min-w-[200px]"><label className="text-[10px] font-bold text-gray-600 mb-1 block uppercase">Pilih Produk</label><CreatableSelect options={allProducts.map(p => ({ value: p.id, label: `${p.nama} (#${p.id})`, dataAsli: p }))} value={selectedProduct} onChange={handleSelectProduct} onCreateOption={handleCreateProductDropdown} placeholder="Ketik nama produk..." /></div>
                  <div className="w-24"><label className="text-[10px] font-bold text-red-600 mb-1 block uppercase">Beli Pabrik</label><div className="flex bg-white border-2 rounded focus-within:border-red-500 overflow-hidden"><input type="text" className="w-full p-2 outline-none text-sm font-bold text-red-700 text-center" value={qtyBeli} onChange={e => setQtyBeli(formatQtyInput(e.target.value))} placeholder="0" /><span className="bg-red-50 text-[10px] font-bold text-red-800 flex items-center px-1 border-l uppercase">{selectedProduct ? (selectedProduct.dataAsli?.satuanBeli || '-') : '-'}</span></div></div>
                  <div className="w-32"><label className="text-[10px] font-bold text-red-600 mb-1 block uppercase">Harga/{selectedProduct ? (selectedProduct.dataAsli?.satuanBeli || '-') : '-'}</label><input type="text" className="w-full border-2 p-2 rounded outline-none focus:border-red-500 text-sm font-bold text-red-700" value={formatRupiahInput(hargaBeli)} onChange={e => setHargaBeli(parseRupiahInput(e.target.value))} placeholder="Rp" /></div>
                  <div className="w-28"><label className="text-[10px] font-bold text-green-700 mb-1 block uppercase">Masuk Gudang</label><div className="flex bg-white border-2 rounded focus-within:border-green-500 overflow-hidden"><input type="text" className="w-full p-2 outline-none text-sm font-bold text-green-700 text-center" value={qtyJual} onChange={e => setQtyJual(formatQtyInput(e.target.value))} placeholder="0" /><span className="bg-green-50 text-[10px] font-bold text-green-800 flex items-center px-1 border-l uppercase">{selectedProduct ? (selectedProduct.dataAsli?.satuanJual || '-') : '-'}</span></div></div>
                ...
                <div className="border rounded-lg overflow-hidden bg-white"><table className="w-full text-xs text-left"><thead className="bg-gray-100 border-b"><tr><th className="p-2">Produk</th><th className="p-2 text-center text-red-700">Qty Pabrik</th><th className="p-2 text-center text-green-700">Masuk Gudang</th><th className="p-2 text-right">Harga Pabrik</th><th className="p-2 text-right">Subtotal</th><th className="p-2 text-center">Del</th></tr></thead><tbody className="divide-y">{purchaseForm.items.length === 0 && <tr><td colSpan="6" className="p-3 text-center text-gray-400 italic">Belum ada barang diinput.</td></tr>}{purchaseForm.items.map((i, idx) => (<tr key={idx} className="hover:bg-gray-50"><td className="p-2 font-bold text-gray-800">{i.nama} <span className="text-[9px] text-gray-400">(#{i.productId})</span></td><td className="p-2 text-center text-red-700 font-bold bg-red-50/50">{i.qtyBeli} <span className="text-[9px] font-normal uppercase">{i.satuanBeli}</span></td><td className="p-2 text-center text-green-700 font-bold bg-green-50/50">{i.qty} <span className="text-[9px] font-normal uppercase">{i.satuanJual}</span></td><td className="p-2 text-right text-gray-600">{formatRp(i.hargaBeli)} <span className="text-[9px] uppercase">/{i.satuanBeli}</span></td><td className="p-2 text-right font-black text-blue-900">{formatRp(i.subtotal)}</td><td className="p-2 text-center"><button onClick={() => setPurchaseForm(prev => ({ ...prev, items: prev.items.filter((_, index) => index !== idx) }))} className="text-red-500 hover:bg-red-50 p-1 rounded"><Trash2 size={14}/></button></td></tr>))}</tbody></table></div>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 mt-4">
                <div className="flex-1 space-y-3">
                  <div><label className="text-[10px] font-bold text-gray-600 mb-1 block uppercase">Catatan Tambahan</label><textarea className="w-full border-2 p-2 rounded-xl h-[72px] outline-none text-xs focus:border-blue-500 resize-none" value={purchaseForm.keterangan} onChange={e => setPurchaseForm({ ...purchaseForm, keterangan: e.target.value })} placeholder="Ketik catatan..."></textarea></div>
                  <div className="bg-gray-50 p-3 rounded-xl border border-gray-200"><label className="text-[10px] font-bold text-gray-700 mb-1.5 flex items-center gap-1 uppercase"><LinkIcon size={12} className="text-blue-500"/> Link Foto Nota / Surat Jalan</label><textarea placeholder="Paste link Drive...&#10;(Pisahkan dgn SPASI)" value={purchaseForm.buktiNota || ''} onChange={(e) => setPurchaseForm(prev => ({ ...prev, buktiNota: e.target.value }))} className="text-xs w-full bg-white p-2.5 rounded-lg border-2 outline-none focus:border-blue-500 h-[65px] resize-none"></textarea></div>
                </div>
                <div className="flex-1 bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3">
                  <div className="flex justify-between font-bold text-gray-600 text-xs border-b pb-2"><span>Total Tagihan:</span><span className="font-black text-sm">{formatRp(totalTagihanPurchase)}</span></div>
                  <div className="flex justify-between font-bold text-green-700 items-center gap-2"><span className="text-[11px] uppercase whitespace-nowrap"> Dibayar Langsung:</span><input type="text" className="w-1/2 border-2 p-2 rounded-lg text-right font-black bg-green-50 outline-none focus:border-green-500 text-sm" value={formatRupiahInput(purchaseForm.bayar)} onChange={e => setPurchaseForm({ ...purchaseForm, bayar: parseRupiahInput(e.target.value) })} placeholder="0" /></div>
                  {parseRupiahInput(purchaseForm.bayar) > 0 && (<div className="grid grid-cols-2 gap-3 pt-2 border-t border-dashed border-gray-200"><div><label className="text-[9px] font-bold text-gray-500 uppercase block mb-1">Metode</label><select className="w-full border-2 p-2 rounded-lg outline-none text-xs focus:border-green-500 font-bold text-gray-700" value={purchaseForm.metodeBayar} onChange={e => setPurchaseForm({ ...purchaseForm, metodeBayar: e.target.value })}><option value="TF">Transfer</option><option value="CASH">Cash / Tunai</option></select></div><div><label className="text-[9px] font-bold text-gray-500 uppercase block mb-1">Link Bukti TF</label><input type="text" className="w-full border-2 p-2 rounded-lg outline-none text-xs focus:border-green-500" value={purchaseForm.buktiTf} onChange={e => setPurchaseForm({ ...purchaseForm, buktiTf: e.target.value })} placeholder="Paste link..." /></div></div>)}
                  <div className="flex justify-between font-black text-red-600 text-sm bg-red-50 p-2.5 rounded-lg border border-red-200 mt-2"><span>SISA HUTANG:</span><span>{formatRp(totalTagihanPurchase - parseRupiahInput(purchaseForm.bayar))}</span></div>
                </div>
              </div>
            </div>
