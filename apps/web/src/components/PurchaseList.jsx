...
const PurchaseList = () => {
  ...
  const handleSimpanBarangMasuk = async () => {
    ...
    try {
      const payload = {
        supplierId: form.supplier.value, items: form.items, tanggal: form.tanggal || new Date().toISOString(),
        tanggalJatuhTempo: form.tglJatuhTempo || null, totalBayar: parseRupiahInput(form.bayar),
        metodeBayar: form.metodeBayar, buktiTf: form.buktiTf, keterangan: form.keterangan, buktiNota: form.buktiNota
      };
      if (form.id) {
  ...
  const handlePay = async () => {
    try {
      await axios.put(`${baseURL}/api/purchases/${payModal.id}/payment`, { totalBayar: parseRupiahInput(payAmount) });
              ...
              {filteredPurchases.map(p => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-3 text-center">
                    {p.buktiNota ? (<a href={p.buktiNota.startsWith('http') ? p.buktiNota : `https://${p.buktiNota}`} target="_blank" rel="noreferrer" className="text-blue-500 hover:text-blue-700 bg-blue-50 p-1.5 rounded-lg inline-flex" title="Lihat Foto Surat Jalan"><ImageIcon size={18}/></a>) : <span className="text-gray-300">-</span>}
                  </td>
                  <td className="p-3 text-xs text-gray-600">
                    {p.items.map((i, idx) => (
                      <div key={idx} className="mb-1">{i.product?.nama} <span className="text-[9px] text-gray-400">(#{i.productId})</span> <span className="font-bold">x{i.qty}</span></div>
                    ))}
                  </td>
                  <td className="p-3 text-right font-medium">{formatRp(p.totalTagihan)}</td>
                  <td className="p-3 text-right text-green-600">{formatRp(p.totalBayar)}</td>
                  <td className="p-3 text-right font-bold text-red-600">{formatRp(p.sisaTagihan)}</td>
                  <td className="p-3 text-center">
                    {p.sisaTagihan <= 0 ? (
            ...
            <div className="p-6 space-y-6">
              {form.id && (
                <div className="bg-yellow-50 border border-yellow-200 p-3 rounded-lg text-[11px] text-yellow-800 font-medium">
                  ⚠ <strong className="font-black">PERHATIAN REVISI:</strong> Menyimpan perubahan di form ini akan otomatis menyesuaikan ulang Stok Gudang. Pastikan qty revisi sudah benar.
                </div>
              )}
              ...
              <div className="bg-blue-50 p-5 rounded-xl border border-blue-200">
                ...
                <div className="flex gap-2 items-end mb-4">
                  <div className="flex-1"><label className="text-[10px] font-bold text-gray-600 mb-1 block">Cari / Buat Produk</label><CreatableSelect options={products} value={selectedProduct} onChange={handleSelectProduct} onCreateOption={handleCreateProduct} placeholder="Ketik nama produk / ID..." /></div>
                  <div className="w-24">
                    <label className="text-[10px] font-bold text-red-600 mb-1 block uppercase">Beli Pabrik</label>
                    <div className="flex bg-white border-2 rounded"><input type="text" className="w-full p-2 outline-none text-xs font-bold text-red-700 text-center" value={qtyBeli} onChange={e => setQtyBeli(formatQtyInput(e.target.value))} placeholder="0" /><span className="bg-red-50 text-[9px] font-bold text-red-800 flex items-center px-1 border-l uppercase">{selectedProduct ? (selectedProduct.dataAsli?.satuanBeli || '-') : '-'}</span></div>
                  </div>
                  <div className="w-32">
                    ...
                    <input type="text" className="w-full border-2 p-2 rounded outline-none text-xs font-bold text-red-700 focus:border-red-500" value={formatRupiahInput(hargaBeli)} onChange={e => setHargaBeli(parseRupiahInput(e.target.value))} placeholder="Rp" />
                  </div>
                  <div className="w-24">
                    <label className="text-[10px] font-bold text-green-700 mb-1 block uppercase">Masuk Gudang</label>
                    <div className="flex bg-white border-2 rounded"><input type="text" className="w-full p-2 outline-none text-xs font-bold text-green-700 text-center" value={qtyJual} onChange={e => setQtyJual(formatQtyInput(e.target.value))} placeholder="0" /><span className="bg-green-50 text-[9px] font-bold text-green-800 flex items-center px-1 border-l uppercase">{selectedProduct ? (selectedProduct.dataAsli?.satuanJual || '-') : '-'}</span></div>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 rounded-xl border border-gray-200">
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-bold text-gray-600 mb-1 block uppercase">Catatan Tambahan</label>
                    <textarea className="w-full border-2 p-2 rounded-xl h-[72px] outline-none text-xs focus:border-blue-500 resize-none" value={form.keterangan} onChange={e => setForm({ ...form, keterangan: e.target.value })} placeholder="Ketik catatan..."></textarea>
                  </div>
                  <div className="bg-white p-3 rounded-lg border">
                    <label className="text-[10px] font-bold text-gray-700 mb-1.5 flex items-center gap-1 uppercase"><LinkIcon size={12} className="text-blue-500"/> Link Foto Nota / Surat Jalan</label>
                    <textarea placeholder="Paste link Drive...&#10;(Pisahkan dgn SPASI)" value={form.buktiNota || ''} onChange={(e) => setForm(prev => ({ ...prev, buktiNota: e.target.value }))} className="text-xs w-full bg-gray-50 p-2.5 rounded-lg border outline-none focus:border-blue-500 h-[65px] resize-none"></textarea>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3">
                  ...
                  <div className="flex justify-between font-bold text-green-700 items-center gap-2">
                    ...
                    <input type="text" className="w-1/2 border-2 p-2 rounded-lg text-right font-black bg-green-50 outline-none focus:border-green-500 text-sm" value={formatRupiahInput(form.bayar)} onChange={e => setForm({ ...form, bayar: parseRupiahInput(e.target.value) })} placeholder="0" />
                  </div>
                  {parseRupiahInput(form.bayar) > 0 && (
                  ...
                  <div className="flex justify-between font-black text-red-600 text-sm bg-red-50 p-2.5 rounded-lg border border-red-200 mt-2">
                    ...
                    <span>{formatRp(totalTagihan - parseRupiahInput(form.bayar))}</span>
                  </div>
                </div>
              </div>
      ...
      {payModal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[10000] p-4">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-[400px]">
            ...
            <input type="text" className="w-full border-2 p-3 rounded-lg font-bold text-green-700 mb-1 focus:outline-green-500 text-lg" value={formatRupiahInput(payAmount)} onChange={e => setPayAmount(parseRupiahInput(e.target.value))} />
          ...
          <div className="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-[400px]">
            <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-3">
              <div>
                ...
                <p className="text-[10px] text-gray-500 font-semibold uppercase mt-1">Pabrik/Supplier: <span className="text-blue-600">{noteModal?.nama}</span></p>
              </div>
