...
const CustomerList = () => {
  ...
  const [form, setForm] = useState({ id: null, nama: '', alamat: '', kontak: '', ongkirDefault: '', ongkirPerusahaanDefault: '', catatan: '', categoryId: '' });
  ...
  const fetchProducts = () => {
    axios.get(`${baseURL}/api/products`).then(res => setProducts(res.data)).catch(e => console.error(e));
  };

  const fetchCategories = () => {
    axios.get(`${baseURL}/api/customers/categories`).then(res => setCategories(res.data)).catch(e => console.error(e));
  };
  ...
  const handleDeleteCategory = async (id, nama) => {
    ...
    try {
      ...
      alert('Gagal! Kategori mungkin masih dipakai oleh pelanggan.'); 
    } finally { 
      setIsLoading(false); 
    }
  };
  ...
  const openAddModal = () => { 
    setForm({ id: null, nama: '', alamat: '', kontak: '', ongkirDefault: '', ongkirPerusahaanDefault: '', catatan: '', categoryId: '' });
    setIsEditing(false);
    setIsModalOpen(true); 
  };

  const openEditModal = (c) => { 
    setForm({ id: c.id, nama: c.nama, alamat: c.alamat, kontak: c.kontak, ongkirDefault: c.ongkirDefault, ongkirPerusahaanDefault: c.ongkirPerusahaanDefault, catatan: c.catatan, categoryId: c.categoryId || '' }); 
    setIsEditing(true);
    setIsModalOpen(true); 
  };
  ...
  const handleSaveSpecialPrice = async () => {
    ...
    try {
      ...
      alert("Gagal menyimpan harga khusus"); 
    } finally { 
      setIsLoading(false); 
    }
  };
  ...
  const handleRemoveSpecialPrice = async (id) => {
    setIsLoading(true);
    try { 
      await axios.delete(`${baseURL}/api/customers/special-price/${id}`);
      fetchCustomers(); 
      setSpecialPriceModal(prev => ({ ...prev, isOpen: false })); 
    } catch(e) { 
      alert("Gagal menghapus harga khusus"); 
    } finally { 
      setIsLoading(false); 
    }
  };
    ...
    <div className="flex flex-col h-full space-y-4">
      ...
      <div className="bg-white p-4 border flex flex-col xl:flex-row justify-between items-center gap-4 rounded-xl shadow-sm shrink-0">
        <div className="flex flex-wrap gap-2 w-full xl:w-auto">
          <button onClick={openAddModal} className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg font-bold flex justify-center items-center gap-2 shadow-sm transition-transform active:scale-95 text-xs md:text-sm whitespace-nowrap">
            <UserPlus size={16} /> Tambah Pelanggan
          </button>
          <button onClick={() => setIsCatModalOpen(true)} className="flex-1 sm:flex-none bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 px-4 py-2.5 rounded-lg font-bold flex justify-center items-center gap-2 text-xs md:text-sm whitespace-nowrap">
            <FolderPlus size={16} /> Kategori
          </button>
          <button onClick={() => setIsRankModalOpen(true)} className="flex-1 sm:flex-none bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2.5 rounded-lg font-bold flex justify-center items-center gap-2 shadow-sm transition-transform active:scale-95 text-xs md:text-sm whitespace-nowrap">
            <Trophy size={16} /> Rank Profit
          </button>
          <button onClick={handleExportExcel} className="flex-1 sm:flex-none bg-green-600 hover:bg-green-700 text-white px-4 py-2.5 rounded-lg font-bold flex justify-center items-center gap-2 shadow-sm transition-transform active:scale-95 text-xs md:text-sm whitespace-nowrap">
        ...
        <table className="w-full text-xs md:text-sm text-left whitespace-nowrap">
          ...
          <tbody className="divide-y border-t-0">
              ...
              return (
                <tr key={c.id} className="hover:bg-blue-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    ...
                    {c.catatan && (
                      <button onClick={() => setNoteModal({ nama: c.nama, text: c.catatan })} className="mt-1.5 text-[10px] text-yellow-700 flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded-md border border-yellow-200 hover:bg-yellow-100 transition-colors cursor-pointer max-w-xs overflow-hidden">
                        <FileText size={12} className="shrink-0" /> <span className="truncate">{c.catatan}</span>
                      </button>
                    )}
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-gray-600">
                    {lastOrder ? (
                      <span className="font-semibold text-blue-700 bg-blue-50 px-3 py-1.5 rounded-md border border-blue-100">
                        {new Date(lastOrder.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </span>
                    ) : (
                  ...
                  <td className="p-4 text-center">
                    <div className="flex justify-center items-center gap-2">
                      ...
                      <button onClick={async () => { if(window.confirm('Hapus pelanggan permanen?')) { await axios.delete(`${baseURL}/api/customers/${c.id}`); fetchCustomers(); } }} className="text-red-500 bg-red-50 hover:bg-red-100 p-2 rounded-lg transition-colors"><Trash2 size={16}/></button>
                    </div>
                  </td>
                </tr>
              );
            ...
            {filteredCustomers.length === 0 && (
              <tr><td colSpan="9" className="p-8 text-center text-gray-400">Tidak ada data pelanggan.</td></tr>
            )}
          </tbody>
        </table>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-[400px]">
            ...
            <div className="p-4">
              <div className="flex gap-2 mb-4">
                <input className="border-2 p-2 rounded-lg flex-1 outline-none focus:border-blue-500 text-xs" placeholder="Nama kategori baru..." value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSaveCategory()} />
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-white border-b sticky top-0">
                    <tr>
                      ...
                      <th className="p-3 text-right">Total Profit Disumbangkan</th>
                    </tr>
                  </thead>
        ...
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[9999] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[500px] flex flex-col overflow-hidden">
            <div className="p-4 border-b flex justify-between items-center bg-gray-50 shrink-0">
              <h3 className="font-bold text-lg text-blue-800 flex items-center gap-2"><UserPlus size={20}/> {isEditing ? 'Edit Pelanggan' : 'Tambah Pelanggan'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="p-1.5 bg-gray-200 rounded-full text-gray-600 hover:text-red-500 transition-colors"><X size={18}/></button>
            </div>
            <div className="p-5 space-y-4 overflow-y-auto">
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1 block">Nama Lengkap / Toko</label>
                <input className="border-2 p-2.5 rounded-lg w-full text-sm font-bold outline-none focus:border-blue-500 uppercase" value={form.nama} onChange={e=>setForm({...form, nama:e.target.value})} placeholder="Wajib diisi..." />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-gray-600 mb-1 block">Kontak / WA</label>
                  <input className="border-2 p-2.5 rounded-lg w-full text-sm outline-none focus:border-blue-500" value={form.kontak} onChange={e=>setForm({...form, kontak:e.target.value})} />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-600 mb-1 block">Daerah</label>
                  <input className="border-2 p-2.5 rounded-lg w-full text-sm outline-none focus:border-blue-500 uppercase" value={form.alamat} onChange={e=>setForm({...form, alamat:e.target.value})} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 border border-gray-200 rounded-xl">
                <div>
                  <label className="text-[10px] font-bold text-gray-500 mb-1 block uppercase tracking-wider">Ongkir dari Customer</label>
                  <input type="text" className="border-2 p-2.5 rounded-lg w-full text-sm font-bold text-green-700 outline-none focus:border-green-500 bg-white" value={formatRupiahInput(form.ongkirDefault)} onChange={e=>setForm({...form, ongkirDefault:parseRupiahInput(e.target.value)})} placeholder="0" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-500 mb-1 block uppercase tracking-wider">Ongkir ke Sopir</label>
                  <input type="text" className="border-2 p-2.5 rounded-lg w-full text-sm font-bold text-red-600 outline-none focus:border-red-500 bg-white" value={formatRupiahInput(form.ongkirPerusahaanDefault)} onChange={e=>setForm({...form, ongkirPerusahaanDefault:parseRupiahInput(e.target.value)})} placeholder="0" />
                </div>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1 block">Catatan Khusus</label>
                <textarea className="border-2 p-2.5 rounded-lg w-full text-sm h-20 outline-none focus:border-blue-500" value={form.catatan} onChange={e=>setForm({...form, catatan:e.target.value})}></textarea>
              </div>
              <div>
            ...
            <div className="p-4 border-t bg-gray-50 flex gap-3 shrink-0">
              ...
              <button onClick={handleSave} disabled={isLoading} className={`flex-1 py-2.5 text-white rounded-lg text-sm font-bold flex justify-center items-center gap-2 transition-transform active:scale-95 disabled:cursor-not-allowed ${isLoading ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'}`}>
      ...
      {specialPriceModal.isOpen && (
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[10000] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[450px] flex flex-col max-h-[90vh] overflow-hidden">
            <div className="p-4 border-b flex justify-between items-center bg-gray-50 shrink-0">
              <h3 className="font-bold text-lg text-yellow-700 flex items-center gap-2"><Tag size={20}/> Harga Khusus</h3>
              <button onClick={() => setSpecialPriceModal({ isOpen: false, customer: null })} className="p-1.5 bg-gray-200 hover:bg-gray-300 rounded-full transition-colors"><X size={18}/></button>
            </div>
            <div className="p-5 overflow-y-auto flex-1">
              <p className="text-sm font-bold text-gray-900 mb-4 bg-gray-100 p-2.5 rounded-lg text-center uppercase tracking-wide border border-gray-200">
                {specialPriceModal.customer.nama} <span className="text-blue-600">(#{specialPriceModal.customer.id})</span>
              </p>
              <div className="bg-yellow-50/50 p-4 rounded-xl border border-yellow-200 mb-5">
                ...
                <select className="w-full border-2 p-2.5 rounded-lg bg-white text-sm outline-none focus:border-yellow-500 font-medium" value={spForm.productId} onChange={e=>setSpForm({...spForm, productId: e.target.value})}>
                  <option value="">Pilih Produk...</option>
                  {products.map(p => <option key={p.id} value={p.id}>{p.nama} (#{p.id}) (Normal: {formatRp(p.hargaJual)})</option>)}
                </select>
                
                <label className="block text-xs font-semibold text-gray-700 mb-1.5 mt-3">Harga Khusus (Rp)</label>
                <input type="text" className="w-full border-2 p-2.5 rounded-lg text-base font-bold text-blue-700 outline-none focus:border-yellow-500" value={formatRupiahInput(spForm.harga)} onChange={e=>setSpForm({...spForm, harga: parseRupiahInput(e.target.value)})} placeholder="0" />
                ...
                <table className="w-full text-xs text-left whitespace-nowrap">
                  <thead className="bg-gray-50 border-b text-gray-600">
                    <tr>
                      ...
                      <th className="p-3 text-right">Harga Khusus</th>
                      <th className="p-3 text-center">Hapus</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {specialPriceModal.customer.hargaKhusus?.map(hk => (
                      <tr key={hk.id} className="hover:bg-gray-50">
                        <td className="p-3 font-semibold text-gray-800">{hk.product?.nama} <span className="text-[10px] text-gray-500">(#{hk.productId})</span></td>
                        <td className="p-3 text-right text-blue-700 font-bold">{formatRp(hk.harga)}</td>
                        <td className="p-3 text-center">
                          <button onClick={() => handleRemoveSpecialPrice(hk.id)} className="text-red-500 hover:bg-red-50 p-1.5 rounded-md transition-colors"><Trash2 size={14}/></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-[10000] p-4 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-[400px]">
            <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-3">
              <div>
                <h3 className="font-bold text-base text-gray-900 flex items-center gap-2">Catatan Pelanggan <FileText size={18} className="text-yellow-500"/></h3>
                <p className="text-[10px] text-gray-500 font-semibold uppercase mt-1">Pelanggan: <span className="text-blue-600">{noteModal?.nama}</span></p>
              </div>
