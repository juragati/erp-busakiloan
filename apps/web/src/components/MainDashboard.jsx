...
>>>>>>> 72ed70229b1a194541bdd0234fe06fd03bd54fdf
...
const MainDashboard = ({ setActiveTab }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
<<<<<<< HEAD
        const [resO, resP, resF] = await Promise.all([ axios.get(`${baseURL}/api/orders`), axios.get(`${baseURL}/api/purchases`), axios.get(`${baseURL}/api/finance`) ]);
...
<<<<<<< HEAD
  ...
  filteredOrders.forEach(o => { 
    if (o.status === 'TERKIRIM') {
       piutang += ((o.totalHarga + (o.ongkosKirim || 0)) - o.dp); 
    }
    // RUMUS SINKRON 100%
    if (o.status === 'TERKIRIM' || o.status === 'SELESAI') {
      let orderOmsetBarang = 0; let orderModalBarang = 0; let hasValidItem = false;
      o.items.forEach(i => { 
...
<<<<<<< HEAD
  ...
  const omset = totalMasuk; // Sesuai permintaan, Omset ditarik dari Pemasukan Buku Kas
=======
...
<<<<<<< HEAD
  const pendingOrders = orders.filter(o => (o.status === 'MENUNGGU' || o.status === 'DP') && isMatch(o.tanggal)).sort((a,b) => new Date(a.tanggal) - new Date(b.tanggal));
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto p-3 md:p-5 space-y-5 bg-gray-50/30">
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between gap-4 items-start md:items-center shrink-0">
        <div><h1 className="text-xl md:text-2xl font-bold text-gray-900 tracking-tight">Dashboard Analisis</h1><p className="text-xs font-medium text-gray-500 mt-1">Ringkasan finansial dan performa bisnis.</p></div>
=======
...
<<<<<<< HEAD
        <div className="lg:col-span-2 bg-white border border-gray-100 rounded-2xl shadow-sm flex flex-col overflow-hidden">
          <div className="p-5 border-b border-orange-100 bg-orange-50 flex justify-between items-center shrink-0">
            <div><h4 className="font-bold text-sm text-gray-900 flex items-center gap-2"><Clock size={16} className="text-orange-500"/> Antrean Belum Dikirim</h4><p className="text-xs text-gray-500 mt-1"><span className="font-bold text-orange-600">{pendingOrders.length}</span> Pesanan perlu diproses dan dikirim</p></div>
=======
...
<<<<<<< HEAD
              <thead className="bg-gray-50 border-b text-gray-500 font-semibold sticky top-0"><tr><th className="p-4 pl-6">Tanggal</th><th className="p-4">ID & Pelanggan</th><th className="p-4">Rincian Barang</th><th className="p-4 text-right">Tagihan Total</th><th className="p-4 text-center pr-6">Status</th></tr></thead>
=======
...
<<<<<<< HEAD
                    <td className="p-4 text-gray-600 font-medium">{o.items.length > 0 ? `${o.items[0].product?.nama} ${o.items.length > 1 ? `(+${o.items.length - 1} lainnya)` : ''}` : '-'}</td>
                    <td className="p-4 text-right font-bold text-gray-900">{formatRp(o.totalHarga + (o.ongkosKirim||0))}</td>
                    <td className="p-4 text-center pr-6"><span className={`text-[10px] font-bold px-2.5 py-1.5 rounded-lg border whitespace-nowrap shadow-sm ${o.status === 'DP' ? 'bg-amber-100 text-amber-700 border-amber-300' : 'bg-red-100 text-red-700 border-red-300'}`}>{o.status}</span></td>
=======
...
<<<<<<< HEAD
                  <Tooltip formatter={(value) => formatRp(value)} contentStyle={{borderRadius: '12px', fontSize: '13px', fontWeight: 'bold', border: 'none'}} />
=======
...
<<<<<<< HEAD
              <div><p className="text-[10px] font-bold text-red-600 uppercase tracking-wider mb-1 flex items-center gap-1.5"><Truck size={14}/> Total Hutang Pabrik</p><h2 className="text-lg md:text-xl font-bold text-red-700">{formatRp(totalHutangAktif)}</h2></div>
=======
              <div>
...
>>>>>>> 72ed70229b1a194541bdd0234fe06fd03bd54fdf
            ...
            <div className="p-4 overflow-y-auto flex-1">
              <div className="space-y-2.5">
                {debtList.map((d, idx) => (
<<<<<<< HEAD
                  <div key={idx} className="flex justify-between items-center bg-gray-50 p-2.5 rounded-xl border border-gray-100"><span className="text-xs font-bold text-gray-800 uppercase truncate">{d.nama}</span><span className="text-xs font-bold text-red-600">{formatRp(d.sisa)}</span></div>
=======
